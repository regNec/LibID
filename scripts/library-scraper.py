...
def updateLibraryMvnCentral(groupId, artefactId, localRepoDir, errorFile, skipAlphaBeta=True):
    ...
    if numberOfVersions > 0:
        for version in data["response"]["docs"]:
            ...
            libtype = version['p']
            if not (os.path.isfile(os.path.join(localRepoDir, f"{libName}_{version['v']}.aar")) or
                    os.path.isfile(os.path.join(localRepoDir, f"{libName}_{version['v']}.jar"))):
...
def updateLibrary(repoURL, groupId, artefactId, localRepoDir, errorFile, skipAlphaBeta=True):
    ...
    if numberOfVersions > 0:
        for version in versions:
            ...
            # skip lib version if already existing
            libtype = 'Unknown'
            if not (os.path.isfile(os.path.join(localRepoDir, f"{libName}_{version}.aar")) or
                    os.path.isfile(os.path.join(localRepoDir, f"{libName}_{version}.jar"))):
            # if not os.path.isfile(baseDirName + "/" + version["v"] + "/" + LIB_DESCRIPTOR_FILE_NAME):
                numberOfUpdates += 1
                date = 'N/A'
